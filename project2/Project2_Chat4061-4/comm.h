...
typedef struct _userInfo {
	int m_pid; 											// child ID
	int m_user_pid;									// user process pid
	char m_user_id[MAX_USER_ID];
	int m_fd_to_user;									//	pipe_SERVER_writing_to_child[idx][1]
	int m_fd_to_server;									// pipe_SERVER_reading_from_child[idx][0]
	int m_status;
} USER;

int recv_fd(int socket, int n, int* fds);
void send_fd(int socket, int *fds, int n);
// modified to add user pid
...
// modified to add user pid
int get_connection(char * user_id, pid_t *user_pid, int pipe_child_writing_to_user[2], int pipe_child_reading_from_user[2]);
