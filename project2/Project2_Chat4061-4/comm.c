...
int connect_to_server(char * connect_point, char * user_id, int pipe_user_reading_from_server[2], int pipe_user_writing_to_server[2])
{
	...
	// +++ Add here
	// add pid of the user process
	int pid = getpid();
	char c_pid[100];
	snprintf(c_pid, sizeof(c_pid), "%d",pid);
	if (write(server_fd, c_pid, sizeof(c_pid)) == -1) {
		perror("Failed to write user pid\n");
		return -1;
	}
		// +++  End Add here
...
int get_connection(char * user_id, int * user_pid, int pipe_child_writing_to_user[2], int pipe_child_reading_from_user[2])
{
	...
	if (cfd != -1) {
		...
		// +++ Add here
		// Get pid of the user process
		if(read(cfd, c_pid, sizeof(c_pid)) == -1) {
			perror("Failed to get user pid");
			return -1;
		}
		*user_pid = atoi (c_pid);
		// +++ END Add here
